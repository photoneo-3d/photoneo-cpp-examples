...
void ApplyCustomProjectionExample::ChangeCameraAngleExample() {
    ...
    if (cameraSpaceFrameId < 0) {
        std::cout << "Trigger camera space frame was not successful, code=" << cameraSpaceFrameId << std::endl;
        return;
    }
    else {
    ...
    if (customSpaceFrameId < 0) {
        std::cout << "Trigger custom rotated frame was not successful, code=" << cameraSpaceFrameId << std::endl;
        return;
    }
    else {
    ...
    pho::api::float32_t k1 = 0.0, k2 = 0.0, p1 = 0.0, p2 = 0.0, k3 = 0.0, k4 = 0.0, k5 = 0.0, k6 = 0.0;
    if (calibrationSettings.DistortionCoefficients.size() >= 8) {
        k1 = (float)calibrationSettings.DistortionCoefficients[0];
        k2 = (float)calibrationSettings.DistortionCoefficients[1];
        p1 = (float)calibrationSettings.DistortionCoefficients[2];
        p2 = (float)calibrationSettings.DistortionCoefficients[3];
        k3 = (float)calibrationSettings.DistortionCoefficients[4];
        k4 = (float)calibrationSettings.DistortionCoefficients[5];
        k5 = (float)calibrationSettings.DistortionCoefficients[6];
        k6 = (float)calibrationSettings.DistortionCoefficients[7];
    }

    pho::api::float32_t fx = 0.0, fy = 0.0, cx = 0.0, cy = 0.0;
    if (!calibrationSettings.CameraMatrix.Empty()) {
        fx = (float)calibrationSettings.CameraMatrix[0][0];
        fy = (float)calibrationSettings.CameraMatrix[1][1];
        cx = (float)calibrationSettings.CameraMatrix[0][2];
        cy = (float)calibrationSettings.CameraMatrix[1][2];
    }
    ...
    for (int y = 0; y < depthMapAutoMinMax.Size.Height; ++y) {
        for (int x = 0; x < depthMapAutoMinMax.Size.Width; ++x) {
            ...
            depthMapAutoMinMax.At(y, x) /= (float)rangeConstant;
        }
    }
