...
private:
	...
	void ColorCameraCalibrationSettingsExample();
...
void ChangeSettingsExample::ConnectPhoXiDeviceBySerialExample()
{
...
void ChangeSettingsExample::ChangeCapturingSettingsExample()
{
	...
	//Check if the SupportedSinglePatternExposures have been changed succesfully
	if (!PhoXiDevice->SupportedSinglePatternExposures.isLastOperationSuccessful())
	{
		throw std::runtime_error(PhoXiDevice->SupportedSinglePatternExposures.GetLastErrorMessage().c_str());
	}
	...
	PhoXiDevice->CapturingSettings->HardwareTrigger = false;
...
void ChangeSettingsExample::ChangeProcessingSettingsExample()
{
	...
	PhoXiDevice->ProcessingSettings->InterreflectionsFiltering = true;

	//InterreflectionFilterStrength values: 0.01-0.99
	PhoXiDevice->ProcessingSettings->InterreflectionFilterStrength = 0.33;
...
void ChangeSettingsExample::ChangeCoordinatesSettingsExample() {
	...
	//Save Transformations values: 0 / 1 or false / true (0 is OFF, 1 is ON)
	PhoXiDevice->CoordinatesSettings->SaveTransformations = 0;
...
void ChangeSettingsExample::CalibrationSettingsExample() {
...
void ChangeSettingsExample::ColorCameraCalibrationSettingsExample() {
	//Retrieving the CalibrationSettings 
...
void ChangeSettingsExample::CorrectDisconnectExample()
{
...
void ChangeSettingsExample::PrintAdditionalCalibrationSettings(const pho::api::PhoXiAdditionalCameraCalibration &CalibrationSettings, const std::string& source)
{
...
void ChangeSettingsExample::PrintCoordinateTransformation(const pho::api::PhoXiCoordinateTransformation& transformation)
{
	PrintMatrix("RotationMatrix", transformation.Rotation);
	PrintVector("TranslationVector", transformation.Translation);
}

void ChangeSettingsExample::PrintResolution(const pho::api::PhoXiSize& Resolution)
{
	std::cout << "    Resolution: ("
		<< Resolution.Width
		<< "x"
...
void ChangeSettingsExample::PrintVector(const std::string &name, const pho::api::Point3_64f &vector)
{
	std::cout << "    " << name << ": ["
		<< vector.x << "; "
		<< vector.y << "; "
...
void ChangeSettingsExample::PrintVirtualCamera(const std::string &name, const pho::api::PhoXiVirtualCamera &camera) {
...
void ChangeSettingsExample::PrintPerspectiveSettings(const pho::api::PhoXiPerspectiveSettings &settings) {
...
void ChangeSettingsExample::PrintDistortionCoefficients(const std::string &name, const std::vector<double> &distCoeffs) {
        std::cout << "    " << name << ": " << std::endl;
...
void ChangeSettingsExample::PrintMatrix(const std::string &name, const pho::api::CameraMatrix64f &matrix)
{
	if (matrix.Empty())
	{
...
void ChangeSettingsExample::Run()
{
	try
	{	//Connecting to scanner
		...
		//Checks
		//Check if the ColorCameraCoordinatesSettings are Enabled and Can be retrieved
		if (!PhoXiDevice->ColorCameraCalibrationSettings.isEnabled() || !PhoXiDevice->ColorCameraCalibrationSettings.CanGet())
		{
			std::cout << "ColorCameraCalibrationSettings not supported by the Device Hardware, or are Read only on the specific device" << std::endl;
			return;
		}
		//End Checks
		std::cout << std::endl;
