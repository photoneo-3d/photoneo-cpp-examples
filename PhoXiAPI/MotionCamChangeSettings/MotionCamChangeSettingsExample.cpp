...
class MotionCamExample {
  private:
    ...
    void ChangeGeneralMotionCamExample();
    ...
    void PrintCalibrationSettings(const pho::api::PhoXiCalibrationSettings &CalibrationSettings, const std::string &source) const;
    void PrintAdditionalCalibrationSettings(const pho::api::PhoXiAdditionalCameraCalibration& CalibrationSettings, const std::string &source) const;
...
void MotionCamExample::ChangeGeneralMotionCamExample() {
    std::cout << std::endl;
    std::cout << "Change General Settings Example" << std::endl;

    // Retrieving the GeneralSettings
    pho::api::PhoXiMotionCam CurrentGeneralSettings = Device->MotionCam.GetValue();
    ...
    pho::api::PhoXiMotionCam BackupGeneralSettings = CurrentGeneralSettings;

    PrintMotionCamGeneral(CurrentGeneralSettings);

    // OperationMode values: Camera / Scanner / Mode2D
    CurrentGeneralSettings.OperationMode = pho::api::PhoXiOperationMode::Scanner;
    ...
    CurrentGeneralSettings.LEDPower = 1650;
    ...
    Device->MotionCam.SetValue(CurrentGeneralSettings);
    // Check if the CurrentGeneralSettings has been set succesfully
    if (!Device->MotionCam.isLastOperationSuccessful()) {
        throw std::runtime_error(Device->MotionCam.GetLastErrorMessage().c_str());
    }

    std::cout << "GeneralSettings have been changed to the following:" << std::endl;
    PrintMotionCamGeneral(CurrentGeneralSettings);

    // Restore previous values
    Device->MotionCam.SetValue(BackupGeneralSettings);
}
...
void MotionCamExample::ChangeMotionCamCameraModeExample() {
    ...
    std::vector<double> SinglePatternExposures = Device->SupportedSinglePatternExposures;
    //Check if the SupportedSinglePatternExposures have been changed succesfully
    if (!Device->SupportedSinglePatternExposures.isLastOperationSuccessful()) {
        throw std::runtime_error(Device->SupportedSinglePatternExposures.GetLastErrorMessage().c_str());
    }
    CurrentCameraMode.Exposure = SinglePatternExposures[0];
    ...
    //TextureSource values: Laser / Color
    CurrentCameraMode.TextureSource = pho::api::PhoXiTextureSource::Laser;
...
void MotionCamExample::ChangeMotionCamScannerModeExample() {
    ...
    CurrentScannerMode.Exposure = SinglePatternExposures[0];
...
void MotionCamExample::ColorCameraCalibrationSettingsExample() {
    std::cout << std::endl;
    std::cout << "Get Calibration Settings Example for the color camera " << std::endl;

    //Retrieving the CalibrationSettings
    pho::api::PhoXiAdditionalCameraCalibration CalibrationSettings = Device->ColorCameraCalibrationSettings.GetValue();
    //Check if the CalibrationSettings have been retrieved succesfully
    if (!Device->CalibrationSettings.isLastOperationSuccessful()) {
        throw std::runtime_error(Device->CalibrationSettings.GetLastErrorMessage().c_str());
    }
...
void MotionCamExample::PrintMotionCamCameraMode(const pho::api::PhoXiMotionCamCameraMode &CameraMode) const {
    ...
    std::cout << "    TextureSource: "      << std::string(CameraMode.TextureSource) << std::endl;
}
...
void MotionCamExample::PrintMotionCamScannerMode(const pho::api::PhoXiMotionCamScannerMode &ScannerMode) const {
    ...
    std::cout << "    Exposure: "           << ScannerMode.Exposure << std::endl;
}
...
void MotionCamExample::PrintCoordinateTransformation(const pho::api::PhoXiCoordinateTransformation& transformation) const {
...
void MotionCamExample::Run() {
    try {
        ...
        if (!Device->ColorCameraCalibrationSettings.isEnabled() || !Device->ColorCameraCalibrationSettings.CanGet()) {
            std::cout << "ColorCameraCalibrationSettings not supported by the Device Hardware" << std::endl;
            return;
        }
