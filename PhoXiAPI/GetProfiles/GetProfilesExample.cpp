...
int main(int argc, char *argv[])
{
    ...
    if (PhoXiDevice->ExportProfile.isLastOperationSuccessful())
    {
        ...
        std::ofstream ExportedProfileFile(ProfileContent.Name + ".phop", std::ios::binary);
        if (ExportedProfileFile.is_open()) {
            const std::vector<unsigned char> &Content = ProfileContent.Content;
            ExportedProfileFile.write((const char*)Content.data(), Content.size());
        }
    }
    else
    {
        std::cout << "Can not export active profile: " << PhoXiDevice->ActiveProfile.GetLastErrorMessage() << std::endl;
        std::cout << "The profile <NON_SELECTED> can not be exported." << std::endl;
        return 0;
    }

    
    //try to load profile from file, if fails, the use profile content from previous import
    std::ifstream ImportProfileFile(ProfileContent.Name + ".phop", std::ios::binary);
    ...
    else
    {
        ...
        std::cout << "The profile <NON_SELECTED> can not be imported." << std::endl;
        return 0;
    }
    ...
    {
        std::cout << "Profile was updated with the current settings." << std::endl;
    }
    else
    {
    ...
    //set active profile as startup profile
    PhoXiDevice->StartupProfile = ActiveProfile;
    if (PhoXiDevice->StartupProfile.isLastOperationSuccessful())
    {
        std::cout << "Profile: " << ActiveProfile << " was set as startup profile." << std::endl;
    }
    else
    {
        std::cout << "Can not set profile: " << ActiveProfile << " as startup profile: " << PhoXiDevice->ImportProfile.GetLastErrorMessage() << std::endl;
        return 0;
    }
